...
public class ArchiveManagerService {
    ...
    public Uni<Response> create(final UriInfo uriInfo,final HttpRequest request) {
        ...
        try
        {
            ...
            if ( content == null )
            {
                return responseHelper.fromResponseReactive("Failed to read historical content which is empty.")
                        .onItem().transform(message -> Response.status(500,message))
                        .onItem().transform(Response.ResponseBuilder::build);
            }
        }
        catch ( final IOException e )
        {
            return responseHelper.fromResponseReactive("Failed to read historical content file from request body.")
                    .onItem().transform(message -> Response.status(500,message))
                    .onItem().transform(Response.ResponseBuilder::build);
        }
        ...
        try
        {
            ...
            if ( archive.isEmpty() )
            {
                return responseHelper.fromResponseReactive("Failed to get downloaded contents for archive.")
                        .onItem().transform(message -> Response.status(500,message))
                        .onItem().transform(Response.ResponseBuilder::build);
            }
        ...
        {
            return responseHelper.fromResponseReactive("Artifacts downloading is interrupted.")
                    .onItem().transform(message -> Response.status(500,message))
                    .onItem().transform(Response.ResponseBuilder::build);
        }
        catch ( final ExecutionException e )
        {
            return responseHelper.fromResponseReactive("Artifacts download execution manager failed.")
                    .onItem().transform(message -> Response.status(500,message))
                    .onItem().transform(Response.ResponseBuilder::build);
        }
        catch ( final IOException e )
        {
            return responseHelper.fromResponseReactive("Failed to generate historical archive from content.")
                    .onItem().transform(message -> Response.status(500,message))
                    .onItem().transform(Response.ResponseBuilder::build);
        }
